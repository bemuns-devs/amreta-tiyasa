...
if ( ! function_exists( 'bemuns_pagination' ) ) {
	/**
	 ...
	 */
	function bemuns_pagination( $args = array(), $class = 'pagination' ) {
		...
		$args = wp_parse_args(
			...
			array(
				'mid_size'           => 2,
				'prev_next'          => true,
				'prev_text'          => __( '&laquo;', 'bemuns' ),
				'next_text'          => __( '&raquo;', 'bemuns' ),
				'type'               => 'array',
				'current'            => max( 1, get_query_var( 'paged' ) ),
				'screen_reader_text' => __( 'Posts navigation', 'bemuns' ),
			)
		);
...
<nav aria-labelledby="posts-nav-label">
	...
	<ul class="<?php echo esc_attr( $class ); ?>">
		...
		<li class="page-item <?php echo strpos( $link, 'current' ) ? 'active' : ''; ?>">
			<?php echo str_replace( 'page-numbers', 'page-link rounded-0 shadow-none', $link ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		</li>
		<?php
				}
				?>
